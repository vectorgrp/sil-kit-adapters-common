...
namespace util {
    namespace detail
    {
        ...
        bool _thereAreUnknownArguments(int argc, char** argv,
            switchesWithArgument_type&& switchesWithArgument,
    ...
    bool thereAreUnknownArguments(int argc, char** argv,
        ...
        switchesWithoutArguments_type&& switchesWithoutArguments)
    {
        return detail::_thereAreUnknownArguments(argc, argv,
            std::forward< switchesWithArgument_type >(switchesWithArgument),
    ...
    {
        return detail::_thereAreUnknownArguments(argc, argv,
            std::forward< std::initializer_list<elementType> >(switchesWithArgument),
    ...
    namespace detail
    {
            ...
            switchesWithoutArguments_type&& switchesWithoutArguments)
        {
            ...
            while (argc)
            {
                ...
                if (std::find_if(switchesWithArgument.begin(), switchesWithArgument.end(), 
                    is_argv) != switchesWithArgument.end())
                {
                    //switches with argument have an argument to ignore, so skip "2"
                    argc -= 2;
                    argv += 2;
                }
                else if (std::find_if(switchesWithoutArguments.begin(), switchesWithoutArguments.end(),
                    is_argv) != switchesWithoutArguments.end())
                {
