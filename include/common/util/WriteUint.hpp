...
namespace adapters { namespace datagram_socket {

enum class Endianness { big_endian, little_endian };

inline std::array<uint8_t, 4> WriteFrameSizeToArray(const uint32_t value, const Endianness endianness, const uint8_t headerSize)
{
    ...
    const uint32_t maxVal = (headerSize == 4)
        ? std::numeric_limits<uint32_t>::max()
        : ((uint32_t{1} << (headerSize * 8)) - 1);
    if (value > maxVal)
    ...
    for (uint8_t i = 0; i < headerSize; ++i)
    {
        ...
        const uint8_t targetIndex = (endianness == Endianness::little_endian)
            ? i
            : static_cast<uint8_t>(headerSize - 1 - i);
        result[targetIndex] = byteValue;
    }
    return result;
}
}} // namespace adapters::datagram_socket
